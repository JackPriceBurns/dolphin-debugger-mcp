...
Built for [SFA-Decomp](https://github.com/zcanann/SFA-Decomp), a *matching*
...
The key really does live under **`[General]`**, not `[Core]` — internally it's
...
## 2. Point the MCP at your SFA-Decomp checkout
...
```sh
export DOLPHIN_REPO_ROOT=/path/to/SFA-Decomp
```

(If you vendor this folder under `<repo>/tools/dolphin_mcp/` it auto-detects the
...
claude mcp add dolphin \
  ...
  -- python3 /path/to/dolphin-debugger-mcp/server.py
```
...
## A/B-ing a value on screen (`toggle_loop`)
...
> Alternate `damagePhase` between 0 and 5 every five seconds while I watch.
...
- `symbols.py` — loads `config/GSAE01/{symbols.txt,splits.txt}` from
  `DOLPHIN_REPO_ROOT`; bisect lookup turns an address into
  `unit::function+offset`. The build byte-matches retail, so symbols.txt
  addresses are the live RAM addresses.
