...
# The symbol/split maps live in an SFA-Decomp checkout. This ships as its own
# repo, so point it at that checkout with DOLPHIN_REPO_ROOT; otherwise fall back
# to a grandparent dir (works when vendored under <repo>/tools/dolphin_mcp/).
REPO_ROOT = Path(
    os.environ.get("DOLPHIN_REPO_ROOT") or Path(__file__).resolve().parents[2]
)
